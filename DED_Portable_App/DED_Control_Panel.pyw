import tkinter as tk
from tkinter import ttk, messagebox, simpledialog
import subprocess
import json
from pathlib import Path
from datetime import datetime, timedelta
import hashlib
import secrets
import webbrowser
import sys
import os
import uuid
import platform
import sqlite3
from werkzeug.security import generate_password_hash

# Import License Control Module
try:
    from license_control import LicenseControl
    LICENSE_CONTROL_AVAILABLE = True
except ImportError:
    LICENSE_CONTROL_AVAILABLE = False
    print("⚠️ Warning: license_control module not found. External control features will be disabled.")

class DEDControlPanel:
    def __init__(self, root):
        self.root = root
        self.root.title("🚀 DED Control Panel - لوحة التحكم الشاملة")
        self.root.geometry("1200x800")
        self.root.resizable(True, True)
        self.root.minsize(1000, 700)

        # Modern Light Theme Colors - واضح وجميل
        self.colors = {
            'bg': '#f8fafc',              # خلفية فاتحة جداً
            'bg_light': '#ffffff',         # أبيض نقي
            'card': '#ffffff',             # بطاقات بيضاء
            'accent': '#3b82f6',          # أزرق حديث
            'accent_hover': '#2563eb',    # أزرق داكن عند التمرير
            'success': '#22c55e',         # أخضر واضح
            'success_hover': '#16a34a',   # أخضر داكن
            'danger': '#ef4444',          # أحمر واضح
            'danger_hover': '#dc2626',    # أحمر داكن
            'warning': '#f59e0b',         # برتقالي
            'warning_hover': '#d97706',   # برتقالي داكن
            'info': '#06b6d4',            # سماوي
            'text': '#1e293b',            # نص داكن واضح
            'text_gray': '#64748b',       # نص رمادي
            'text_muted': '#94a3b8',      # نص باهت
            'text_secondary': '#64748b',  # نص ثانوي
            'border': '#e2e8f0',          # حدود فاتحة
            'green_line': '#22c55e',      # خط أخضر
            'purple_tab': '#a855f7',      # بنفسجي
            'shadow': '#00000010'          # ظل خفيف
        }

        self.root.configure(bg=self.colors['bg'])

        # App directory - use script directory instead of cwd
        if getattr(sys, 'frozen', False):
            # Running as compiled executable
            self.app_dir = Path(sys.executable).parent
        else:
            # Running as script
            self.app_dir = Path(__file__).parent

        self.license_file = self.app_dir / "licenses.json"
        self.licenses = self.load_licenses()

        # Initialize License Control
        if LICENSE_CONTROL_AVAILABLE:
            try:
                self.license_control = LicenseControl()
                print("✅ License Control initialized successfully")
            except Exception as e:
                self.license_control = None
                print(f"⚠️ Failed to initialize License Control: {e}")
        else:
            self.license_control = None

        # Flask process
        self.flask_process = None
        self.is_running = False

        # Build UI
        self.create_ui()
        self.center_window()

        # Check status
        self.root.after(500, self.check_status)

        # Auto-sync with database every 30 seconds
        self.auto_sync_interval = 30000  # 30 seconds in milliseconds
        self.root.after(5000, self.auto_sync)  # First sync after 5 seconds

        # Bind close event
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def center_window(self):
        self.root.update_idletasks()
        width = self.root.winfo_width()
        height = self.root.winfo_height()
        x = (self.root.winfo_screenwidth() // 2) - (width // 2)
        y = (self.root.winfo_screenheight() // 2) - (height // 2)
        self.root.geometry(f'{width}x{height}+{x}+{y}')

    def load_licenses(self):
        # Debug: print paths
        print(f"App dir: {self.app_dir}")
        print(f"License file: {self.license_file}")
        print(f"File exists: {self.license_file.exists()}")

        if self.license_file.exists():
            try:
                with open(self.license_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    print(f"Loaded {len(data)} licenses")
                    return data
            except Exception as e:
                print(f"Error loading licenses: {e}")
                return {}
        else:
            print("License file does not exist!")
        return {}

    def save_licenses(self):
        with open(self.license_file, 'w', encoding='utf-8') as f:
            json.dump(self.licenses, f, indent=2, ensure_ascii=False)

    def create_ui(self):
        # Main container with gradient effect
        main = tk.Frame(self.root, bg=self.colors['bg'])
        main.pack(fill=tk.BOTH, expand=True)

        # Header with shadow effect
        header = tk.Frame(main, bg=self.colors['bg_light'], height=120)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        # Add subtle shadow line
        tk.Frame(main, bg=self.colors['border'], height=1).pack(fill=tk.X)

        header_content = tk.Frame(header, bg=self.colors['bg_light'])
        header_content.pack(expand=True, pady=20)

        # Logo with gradient background
        logo_frame = tk.Frame(header_content, bg=self.colors['accent'], width=80, height=80)
        logo_frame.pack(side=tk.LEFT, padx=(30, 20))
        logo_frame.pack_propagate(False)

        tk.Label(
            logo_frame,
            text="🚀",
            font=("Segoe UI Emoji", 45),
            bg=self.colors['accent'],
            fg='white'
        ).pack(expand=True)

        # Title section
        title_frame = tk.Frame(header_content, bg=self.colors['bg_light'])
        title_frame.pack(side=tk.LEFT)

        tk.Label(
            title_frame,
            text="DED Control Panel",
            font=("Segoe UI", 28, "bold"),
            bg=self.colors['bg_light'],
            fg=self.colors['text']
        ).pack(anchor='w')

        tk.Label(
            title_frame,
            text="لوحة التحكم الشاملة - نظام إدارة متكامل وسهل الاستخدام",
            font=("Segoe UI", 12),
            bg=self.colors['bg_light'],
            fg=self.colors['text_gray']
        ).pack(anchor='w', pady=(5, 0))

        # Decorative line
        tk.Frame(main, bg=self.colors['green_line'], height=4).pack(fill=tk.X)

        # Tabs
        self.create_tabs(main)

    def create_tabs(self, parent):
        # Tab container with padding
        tab_container = tk.Frame(parent, bg=self.colors['bg'])
        tab_container.pack(fill=tk.BOTH, expand=True, padx=20, pady=15)

        # Tab buttons frame with modern design
        tab_buttons = tk.Frame(tab_container, bg=self.colors['bg'])
        tab_buttons.pack(fill=tk.X, pady=(0, 15))

        # Tab button 1 - App Control (larger and clearer)
        self.app_tab_btn = tk.Button(
            tab_buttons,
            text="📱 تشغيل التطبيق\nApp Control",
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['accent'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.switch_tab(0),
            padx=30,
            pady=15,
            borderwidth=0,
            activebackground=self.colors['accent_hover'],
            activeforeground='white'
        )
        self.app_tab_btn.pack(side=tk.LEFT, padx=(0, 10))

        # Tab button 2 - License Manager (larger and clearer)
        self.license_tab_btn = tk.Button(
            tab_buttons,
            text="🔐 مدير التراخيص\nLicense Manager",
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['text_gray'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.switch_tab(1),
            padx=30,
            pady=15,
            borderwidth=0,
            activebackground=self.colors['purple_tab'],
            activeforeground='white'
        )
        self.license_tab_btn.pack(side=tk.LEFT, padx=(0, 10))

        # Content frame with card design
        self.content_frame = tk.Frame(tab_container, bg=self.colors['bg'])
        self.content_frame.pack(fill=tk.BOTH, expand=True)

        # Create tab contents
        self.app_tab = tk.Frame(self.content_frame, bg=self.colors['bg'])
        self.license_tab = tk.Frame(self.content_frame, bg=self.colors['bg'])

        self.create_app_tab()
        self.create_license_tab()

        # Show first tab
        self.current_tab = 0
        self.switch_tab(0)

    def switch_tab(self, tab_index):
        # Hide all tabs
        self.app_tab.pack_forget()
        self.license_tab.pack_forget()

        # Reset button colors to inactive state
        self.app_tab_btn.config(bg=self.colors['text_gray'], fg='white')
        self.license_tab_btn.config(bg=self.colors['text_gray'], fg='white')

        # Show selected tab with active color
        if tab_index == 0:
            self.app_tab.pack(fill=tk.BOTH, expand=True)
            self.app_tab_btn.config(bg=self.colors['accent'], fg='white')
        else:
            self.license_tab.pack(fill=tk.BOTH, expand=True)
            self.license_tab_btn.config(bg=self.colors['purple_tab'], fg='white')

        self.current_tab = tab_index

    def create_app_tab(self):
        # Status Card with modern design
        status_card = tk.Frame(self.app_tab, bg=self.colors['card'], relief=tk.FLAT, borderwidth=0)
        status_card.pack(fill=tk.X, padx=10, pady=(0, 20))

        # Add border effect
        tk.Frame(status_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        # Card header
        tk.Label(
            status_card,
            text="📊 حالة النظام - System Status",
            font=("Segoe UI", 18, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(pady=(20, 10))

        # Status indicator with larger font
        self.status_label = tk.Label(
            status_card,
            text="⚫ متوقف - Stopped",
            font=("Segoe UI", 24, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text_gray']
        )
        self.status_label.pack(pady=15)

        # URL with better visibility
        self.url_label = tk.Label(
            status_card,
            text="🌐 http://127.0.0.1:5000",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['card'],
            fg=self.colors['info'],
            cursor="hand2"
        )
        self.url_label.pack(pady=(0, 20))
        self.url_label.bind("<Button-1>", lambda e: self.open_browser())

        # Bottom border
        tk.Frame(status_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        # Control Buttons with modern card design
        btn_frame = tk.Frame(self.app_tab, bg=self.colors['bg'])
        btn_frame.pack(pady=10, padx=10, fill=tk.X)

        # Start button - larger and clearer
        self.start_btn = tk.Button(
            btn_frame,
            text="▶️ تشغيل التطبيق\nStart Application",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.start_app,
            width=25,
            height=4,
            borderwidth=0,
            activebackground=self.colors['success_hover'],
            activeforeground='white'
        )
        self.start_btn.grid(row=0, column=0, padx=10, pady=5, sticky='ew')

        # Stop button - larger and clearer
        self.stop_btn = tk.Button(
            btn_frame,
            text="⏹️ إيقاف التطبيق\nStop Application",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.stop_app,
            width=25,
            height=4,
            state='disabled',
            borderwidth=0,
            activebackground=self.colors['danger_hover'],
            activeforeground='white'
        )
        self.stop_btn.grid(row=0, column=1, padx=10, pady=5, sticky='ew')

        # Open browser button - larger and clearer
        tk.Button(
            btn_frame,
            text="🌐 فتح المتصفح\nOpen Browser",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['accent'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.open_browser,
            width=25,
            height=4,
            borderwidth=0,
            activebackground=self.colors['accent_hover'],
            activeforeground='white'
        ).grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky='ew')

        # Make columns expand equally
        btn_frame.grid_columnconfigure(0, weight=1)
        btn_frame.grid_columnconfigure(1, weight=1)

        # Info Card with modern design
        info_card = tk.Frame(self.app_tab, bg=self.colors['card'], relief=tk.FLAT)
        info_card.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

        # Top border
        tk.Frame(info_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        tk.Label(
            info_card,
            text="ℹ️ معلومات النظام - System Information",
            font=("Segoe UI", 18, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(pady=(20, 15))

        # Info items with better formatting
        info_items = [
            ("🌐", "عنوان التطبيق - URL", "http://127.0.0.1:5000"),
            ("👤", "اسم المستخدم - Username", "admin"),
            ("🔑", "كلمة المرور - Password", "admin123"),
            ("📁", "مسار التطبيق - Path", str(self.app_dir))
        ]

        for icon, label, value in info_items:
            item_frame = tk.Frame(info_card, bg=self.colors['card'])
            item_frame.pack(fill=tk.X, padx=30, pady=8)

            tk.Label(
                item_frame,
                text=f"{icon} {label}:",
                font=("Segoe UI", 13, "bold"),
                bg=self.colors['card'],
                fg=self.colors['text'],
                anchor='w'
            ).pack(side=tk.LEFT, padx=(0, 10))

            tk.Label(
                item_frame,
                text=value,
                font=("Segoe UI", 13),
                bg=self.colors['card'],
                fg=self.colors['info'],
                anchor='w'
            ).pack(side=tk.LEFT)

        # Bottom padding
        tk.Frame(info_card, bg=self.colors['card'], height=20).pack()

        # Bottom border
        tk.Frame(info_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        # License Management Quick Access Card
        license_quick_card = tk.Frame(self.app_tab, bg=self.colors['card'], relief=tk.FLAT)
        license_quick_card.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

        # Top border
        tk.Frame(license_quick_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        tk.Label(
            license_quick_card,
            text="🔐 إدارة التراخيص السريعة - Quick License Management",
            font=("Segoe UI", 18, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(pady=(20, 15))

        # License Statistics Row
        stats_row = tk.Frame(license_quick_card, bg=self.colors['card'])
        stats_row.pack(fill=tk.X, padx=30, pady=(0, 15))

        # Helper function for stat cards
        def create_stat_card(parent, icon, title, value_var_name, color):
            card = tk.Frame(parent, bg=color, relief=tk.FLAT)
            card.pack(side=tk.LEFT, padx=10, expand=True, fill=tk.BOTH)

            tk.Label(
                card,
                text=icon,
                font=("Segoe UI Emoji", 32),
                bg=color,
                fg='white'
            ).pack(pady=(15, 5))

            value_label = tk.Label(
                card,
                text="0",
                font=("Segoe UI", 28, "bold"),
                bg=color,
                fg='white'
            )
            value_label.pack()
            setattr(self, value_var_name, value_label)

            tk.Label(
                card,
                text=title,
                font=("Segoe UI", 11, "bold"),
                bg=color,
                fg='white'
            ).pack(pady=(5, 15))

            return card

        # Create stat cards
        create_stat_card(stats_row, "📊", "إجمالي التراخيص\nTotal Licenses",
                        "total_licenses_label", self.colors['accent'])
        create_stat_card(stats_row, "✅", "تراخيص نشطة\nActive Licenses",
                        "active_licenses_label", self.colors['success'])
        create_stat_card(stats_row, "⏸️", "تراخيص معلقة\nSuspended",
                        "suspended_licenses_label", self.colors['warning'])
        create_stat_card(stats_row, "⚠️", "تراخيص منتهية\nExpired",
                        "expired_licenses_label", self.colors['danger'])

        # Recent Licenses Section
        recent_frame = tk.Frame(license_quick_card, bg=self.colors['card'])
        recent_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=(10, 15))

        tk.Label(
            recent_frame,
            text="📋 آخر التراخيص المضافة - Recent Licenses",
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(anchor='w', pady=(0, 10))

        # Recent licenses list with scrollbar
        recent_list_frame = tk.Frame(recent_frame, bg=self.colors['bg'])
        recent_list_frame.pack(fill=tk.BOTH, expand=True)

        recent_scrollbar = tk.Scrollbar(recent_list_frame)
        recent_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        self.recent_licenses_listbox = tk.Listbox(
            recent_list_frame,
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text'],
            relief=tk.FLAT,
            borderwidth=0,
            yscrollcommand=recent_scrollbar.set,
            height=5,
            selectbackground=self.colors['accent'],
            selectforeground='white'
        )
        self.recent_licenses_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        recent_scrollbar.config(command=self.recent_licenses_listbox.yview)

        # Quick Action Buttons
        quick_actions = tk.Frame(license_quick_card, bg=self.colors['card'])
        quick_actions.pack(fill=tk.X, padx=30, pady=(10, 20))

        tk.Button(
            quick_actions,
            text="➕ إضافة ترخيص جديد\nAdd New License",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.switch_tab(1),
            padx=25,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['success_hover'],
            activeforeground='white'
        ).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        tk.Button(
            quick_actions,
            text="📋 عرض جميع التراخيص\nView All Licenses",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['accent'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.switch_tab(1),
            padx=25,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['accent_hover'],
            activeforeground='white'
        ).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        tk.Button(
            quick_actions,
            text="🔄 تحديث الإحصائيات\nRefresh Stats",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['info'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.update_license_stats,
            padx=25,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['accent'],
            activeforeground='white'
        ).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        # Sync button - NEW!
        tk.Button(
            quick_actions,
            text="🔄 مزامنة\nSync DB",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['warning'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.manual_sync,
            padx=25,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['warning_hover'],
            activeforeground='white'
        ).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

        # Bottom border
        tk.Frame(license_quick_card, bg=self.colors['border'], height=1).pack(fill=tk.X)

        # Update license stats initially
        self.update_license_stats()

    def create_license_tab(self):
        # Main container with scrollbar
        main_container = tk.Frame(self.license_tab, bg=self.colors['bg'])
        main_container.pack(fill=tk.BOTH, expand=True)

        # Create canvas for scrolling
        canvas = tk.Canvas(main_container, bg=self.colors['bg'], highlightthickness=0)
        scrollbar = tk.Scrollbar(main_container, orient="vertical", command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg=self.colors['bg'])

        scrollable_frame.bind(
            "<Configure>",
            lambda e: canvas.configure(scrollregion=canvas.bbox("all"))
        )

        canvas.create_window((0, 0), window=scrollable_frame, anchor="nw")
        canvas.configure(yscrollcommand=scrollbar.set)

        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # Beautiful Purple Header with gradient effect
        header = tk.Frame(scrollable_frame, bg='#7c3aed', height=120)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        header_content = tk.Frame(header, bg='#7c3aed')
        header_content.pack(expand=True, fill=tk.BOTH, padx=40, pady=20)

        # Left side - Icon and Title
        left_side = tk.Frame(header_content, bg='#7c3aed')
        left_side.pack(side=tk.LEFT, fill=tk.Y)

        tk.Label(
            left_side,
            text="⚙️",
            font=("Segoe UI Emoji", 32),
            bg='#7c3aed',
            fg='white'
        ).pack(side=tk.LEFT, padx=(0, 15))

        title_frame = tk.Frame(left_side, bg='#7c3aed')
        title_frame.pack(side=tk.LEFT)

        tk.Label(
            title_frame,
            text="إدارة التراخيص",
            font=("Segoe UI", 24, "bold"),
            bg='#7c3aed',
            fg='white'
        ).pack(anchor='w')

        tk.Label(
            title_frame,
            text="نظام إدارة التراخيص المتقدم",
            font=("Segoe UI", 12),
            bg='#7c3aed',
            fg='#e9d5ff'
        ).pack(anchor='w')

        # Right side - New License Button
        tk.Button(
            header_content,
            text="➕ ترخيص جديد",
            font=("Segoe UI", 13, "bold"),
            bg='white',
            fg='#7c3aed',
            relief=tk.FLAT,
            cursor="hand2",
            command=self.open_new_license_dialog,
            padx=25,
            pady=12,
            borderwidth=0,
            activebackground='#f3e8ff',
            activeforeground='#7c3aed'
        ).pack(side=tk.RIGHT)

        # Statistics Cards Section
        stats_container = tk.Frame(scrollable_frame, bg=self.colors['bg'])
        stats_container.pack(fill=tk.X, padx=20, pady=20)

        # Create frame for stat cards
        self.stat_cards_frame = tk.Frame(stats_container, bg=self.colors['bg'])
        self.stat_cards_frame.pack(fill=tk.X)

        # License List Header
        list_header = tk.Frame(scrollable_frame, bg=self.colors['bg'])
        list_header.pack(fill=tk.X, padx=20, pady=(10, 15))

        tk.Label(
            list_header,
            text="📋 جميع التراخيص",
            font=("Segoe UI", 18, "bold"),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(side=tk.LEFT)

        self.license_count_label = tk.Label(
            list_header,
            text="0",
            font=("Segoe UI", 14),
            bg=self.colors['bg'],
            fg=self.colors['text_gray']
        )
        self.license_count_label.pack(side=tk.LEFT, padx=10)

        # License Cards Grid Container
        cards_container = tk.Frame(scrollable_frame, bg=self.colors['bg'])
        cards_container.pack(fill=tk.BOTH, expand=True, padx=20, pady=(0, 20))

        self.licenses_grid_frame = tk.Frame(cards_container, bg=self.colors['bg'])
        self.licenses_grid_frame.pack(fill=tk.BOTH, expand=True)

        # Store reference
        self.selected_license_key = None

        # Load licenses
        self.refresh_list()

    # App Control Methods
    def start_app(self):
        try:
            # Start Flask app
            self.flask_process = subprocess.Popen(
                [sys.executable, "run.py"],
                cwd=self.app_dir,
                creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == 'win32' else 0
            )
            self.is_running = True
            self.update_status()
            messagebox.showinfo("نجح - Success", "تم تشغيل التطبيق!\nApplication started!")
        except Exception as e:
            messagebox.showerror("خطأ - Error", f"فشل تشغيل التطبيق:\n{str(e)}")

    def stop_app(self):
        if self.flask_process:
            try:
                self.flask_process.terminate()
                self.flask_process.wait(timeout=5)
            except:
                self.flask_process.kill()
            self.flask_process = None

        self.is_running = False
        self.update_status()
        messagebox.showinfo("نجح - Success", "تم إيقاف التطبيق!\nApplication stopped!")

    def open_browser(self):
        """Open browser and check if app is running"""
        if not self.is_running:
            response = messagebox.askyesno(
                "تحذير - Warning",
                "⚠️ التطبيق غير مشغّل!\nApplication is not running!\n\n"
                "هل تريد تشغيل التطبيق أولاً؟\n"
                "Do you want to start the application first?"
            )
            if response:
                self.start_app()
                # Wait a moment for the app to start
                self.root.after(2000, lambda: webbrowser.open("http://127.0.0.1:5000"))
            return

        webbrowser.open("http://127.0.0.1:5000")

    def check_status(self):
        # Check if Flask is running
        try:
            import socket
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            result = sock.connect_ex(('127.0.0.1', 5000))
            sock.close()

            if result == 0:
                self.is_running = True
            else:
                self.is_running = False
                if self.flask_process:
                    self.flask_process = None
        except:
            self.is_running = False

        self.update_status()

    def update_status(self):
        if self.is_running:
            self.status_label.config(text="🟢 يعمل - Running", fg=self.colors['success'])
            self.start_btn.config(state='disabled')
            self.stop_btn.config(state='normal')
        else:
            self.status_label.config(text="⚫ متوقف - Stopped", fg=self.colors['text_gray'])
            self.start_btn.config(state='normal')
            self.stop_btn.config(state='disabled')

    # License Management Methods
    def open_new_license_dialog(self):
        """Open a beautiful dialog to add a new license"""
        dialog = tk.Toplevel(self.root)
        dialog.title("➕ إضافة ترخيص جديد - Add New License")
        dialog.geometry("600x700")
        dialog.resizable(False, False)
        dialog.configure(bg=self.colors['bg'])

        # Center the dialog
        dialog.transient(self.root)
        dialog.grab_set()

        # Header with purple gradient
        header = tk.Frame(dialog, bg='#7c3aed', height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="⚙️ إنشاء ترخيص جديد",
            font=("Segoe UI", 20, "bold"),
            bg='#7c3aed',
            fg='white'
        ).pack(pady=25)

        # Form container
        form_container = tk.Frame(dialog, bg=self.colors['bg'])
        form_container.pack(fill=tk.BOTH, expand=True, padx=30, pady=20)

        # Create form fields
        fields = {}

        def create_field(parent, label_text, default_value="", show_char=None):
            field_frame = tk.Frame(parent, bg=self.colors['bg'])
            field_frame.pack(fill=tk.X, pady=8)

            tk.Label(
                field_frame,
                text=label_text,
                font=("Segoe UI", 11, "bold"),
                bg=self.colors['bg'],
                fg=self.colors['text']
            ).pack(anchor='w', pady=(0, 5))

            entry = tk.Entry(
                field_frame,
                font=("Segoe UI", 11),
                bg='white',
                fg=self.colors['text'],
                relief=tk.SOLID,
                borderwidth=1,
                show=show_char if show_char else None
            )
            entry.pack(fill=tk.X, ipady=8)
            if default_value:
                entry.insert(0, default_value)

            return entry

        # Create all fields
        fields['company'] = create_field(form_container, "🏢 اسم الشركة *")
        fields['username'] = create_field(form_container, "👤 اسم المستخدم *")
        fields['password'] = create_field(form_container, "🔒 كلمة المرور *", show_char="*")
        fields['email'] = create_field(form_container, "📧 البريد الإلكتروني")
        fields['phone'] = create_field(form_container, "📱 رقم الهاتف")
        fields['duration'] = create_field(form_container, "⏱️ المدة (بالأيام) *", "365")
        fields['max_users'] = create_field(form_container, "👥 عدد المستخدمين *", "10")
        fields['notes'] = create_field(form_container, "📝 ملاحظات")

        # Buttons frame
        btn_frame = tk.Frame(dialog, bg=self.colors['bg'])
        btn_frame.pack(fill=tk.X, padx=30, pady=(0, 20))

        def create_license_from_dialog():
            # Get values
            company = fields['company'].get().strip()
            username = fields['username'].get().strip()
            password = fields['password'].get().strip()
            email = fields['email'].get().strip()
            phone = fields['phone'].get().strip()
            duration = fields['duration'].get().strip()
            max_users = fields['max_users'].get().strip()
            notes = fields['notes'].get().strip()

            # Validation
            if not company:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال اسم الشركة!\nPlease enter company name!", parent=dialog)
                return

            if not username:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال اسم المستخدم!\nPlease enter username!", parent=dialog)
                return

            if not password:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال كلمة المرور!\nPlease enter password!", parent=dialog)
                return

            try:
                days = int(duration)
                if days <= 0:
                    raise ValueError
            except:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال مدة صحيحة!\nPlease enter valid duration!", parent=dialog)
                return

            try:
                max_users_int = int(max_users)
                if max_users_int <= 0:
                    raise ValueError
            except:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال عدد مستخدمين صحيح!\nPlease enter valid max users!", parent=dialog)
                return

            # Generate license
            machine_id = self.get_machine_id()
            key = self.create_license_key(company, machine_id)
            expiry = (datetime.now() + timedelta(days=days)).strftime("%Y-%m-%d")
            created_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Save license
            self.licenses[key] = {
                'company': company,
                'expiry': expiry,
                'created': created_date,
                'duration_days': days,
                'machine_id': machine_id,
                'license_type': 'Standard',
                'max_users': max_users_int,
                'features': ['all'],
                'status': 'active',
                'activation_count': 0,
                'last_check': None,
                'username': username,
                'password': password,
                'contact_email': email,
                'contact_phone': phone,
                'notes': notes
            }

            self.save_licenses()

            # Use license_control if available, otherwise fallback to old method
            if self.license_control:
                try:
                    # Add to licenses_master.db using license_control
                    success, message = self.add_license_to_master_db(key, self.licenses[key])
                    if success:
                        # Create tenant database
                        self.create_tenant_database(key, username, password)
                        print(f"✅ License added via license_control: {message}")
                    else:
                        print(f"⚠️ Failed to add via license_control: {message}")
                        # Fallback to old method
                        self.sync_license_to_database(key, self.licenses[key])
                except Exception as e:
                    print(f"⚠️ Error using license_control: {e}")
                    # Fallback to old method
                    self.sync_license_to_database(key, self.licenses[key])
            else:
                # Old method
                self.sync_license_to_database(key, self.licenses[key])

            messagebox.showinfo(
                "نجح - Success",
                f"✅ تم إنشاء الترخيص بنجاح!\nLicense created successfully!\n\n"
                f"🏢 الشركة: {company}\n"
                f"📅 تاريخ الانتهاء: {expiry}\n"
                f"🔑 المفتاح:\n{key}",
                parent=dialog
            )

            dialog.destroy()
            self.refresh_list()

        # Create button
        tk.Button(
            btn_frame,
            text="✅ إنشاء الترخيص - Create License",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=create_license_from_dialog,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))

        # Cancel button
        tk.Button(
            btn_frame,
            text="❌ إلغاء - Cancel",
            font=("Segoe UI", 13, "bold"),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=dialog.destroy,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))

    def open_new_license_dialog(self):
        """Open a popup dialog to add a new license"""
        dialog = tk.Toplevel(self.root)
        dialog.title("➕ إضافة ترخيص جديد - Add New License")
        dialog.geometry("700x750")  # Increased height
        dialog.configure(bg=self.colors['bg'])
        dialog.resizable(False, False)

        # Center the dialog
        dialog.transient(self.root)
        dialog.grab_set()

        # Purple header
        header = tk.Frame(dialog, bg='#7c3aed', height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="➕ إنشاء ترخيص جديد",
            font=("Segoe UI", 20, "bold"),
            bg='#7c3aed',
            fg='white'
        ).pack(expand=True)

        # Scrollable container
        scroll_container = tk.Frame(dialog, bg=self.colors['bg'])
        scroll_container.pack(fill=tk.BOTH, expand=True)

        canvas = tk.Canvas(scroll_container, bg=self.colors['bg'], highlightthickness=0)
        scrollbar = tk.Scrollbar(scroll_container, orient="vertical", command=canvas.yview)

        form_container = tk.Frame(canvas, bg=self.colors['bg'])

        form_container.bind(
            "<Configure>",
            lambda e: canvas.configure(scrollregion=canvas.bbox("all"))
        )

        canvas.create_window((0, 0), window=form_container, anchor="nw", width=670)
        canvas.configure(yscrollcommand=scrollbar.set)

        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=30, pady=20)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y, pady=20)

        # Create form fields
        fields = []

        def create_field(label_text, is_required=False, show_char=None, default_value=""):
            field_frame = tk.Frame(form_container, bg=self.colors['bg'])
            field_frame.pack(fill=tk.X, pady=5)  # Reduced padding

            label = tk.Label(
                field_frame,
                text=label_text + (" *" if is_required else ""),
                font=("Segoe UI", 10, "bold"),  # Smaller font
                bg=self.colors['bg'],
                fg=self.colors['text'],
                anchor='w'
            )
            label.pack(fill=tk.X, pady=(0, 3))  # Reduced padding

            entry = tk.Entry(
                field_frame,
                font=("Segoe UI", 10),  # Smaller font
                relief=tk.FLAT,
                bg=self.colors['card'],
                fg=self.colors['text'],
                insertbackground=self.colors['text']
            )
            if show_char:
                entry.config(show=show_char)
            if default_value:
                entry.insert(0, default_value)
            entry.pack(fill=tk.X, ipady=6, ipadx=10)  # Reduced padding

            # Add border effect
            tk.Frame(field_frame, bg=self.colors['border'], height=1).pack(fill=tk.X)

            return entry

        # Create all fields
        company_entry = create_field("اسم الشركة - Company Name", is_required=True)
        username_entry = create_field("اسم المستخدم - Username", is_required=True)
        password_entry = create_field("كلمة المرور - Password", is_required=True, show_char="*")
        duration_entry = create_field("المدة (أيام) - Duration (Days)", is_required=True, default_value="365")
        max_users_entry = create_field("عدد المستخدمين - Max Users", is_required=True, default_value="10")
        email_entry = create_field("البريد الإلكتروني - Email")
        phone_entry = create_field("رقم الهاتف - Phone")
        notes_entry = create_field("ملاحظات - Notes")

        # Buttons
        btn_frame = tk.Frame(dialog, bg=self.colors['bg'])
        btn_frame.pack(fill=tk.X, padx=30, pady=(0, 30))

        def create_license_from_dialog():
            # Set the entry values to the main form entries
            self.company_entry = company_entry
            self.username_entry = username_entry
            self.password_entry = password_entry
            self.duration_entry = duration_entry
            self.max_users_entry = max_users_entry
            self.email_entry = email_entry
            self.phone_entry = phone_entry
            self.notes_entry = notes_entry

            # Call generate_license
            self.generate_license()

            # Close dialog if successful
            if hasattr(self, 'licenses') and len(self.licenses) > 0:
                dialog.destroy()

        tk.Button(
            btn_frame,
            text="✅ إنشاء الترخيص - Create License",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=create_license_from_dialog,
            padx=30,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['success_hover'],
            activeforeground='white'
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))

        tk.Button(
            btn_frame,
            text="❌ إلغاء - Cancel",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['text_gray'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=dialog.destroy,
            padx=30,
            pady=12,
            borderwidth=0,
            activebackground=self.colors['danger'],
            activeforeground='white'
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))

    def generate_license(self):
        company = self.company_entry.get().strip()
        duration = self.duration_entry.get().strip()
        username = self.username_entry.get().strip()
        password = self.password_entry.get().strip()
        email = self.email_entry.get().strip()
        phone = self.phone_entry.get().strip()
        max_users = self.max_users_entry.get().strip()
        notes = self.notes_entry.get().strip()

        # Validation
        if not company:
            messagebox.showerror("خطأ - Error", "الرجاء إدخال اسم الشركة!\nPlease enter company name!")
            return

        if not username:
            messagebox.showerror("خطأ - Error", "الرجاء إدخال اسم المستخدم!\nPlease enter username!")
            return

        if not password:
            messagebox.showerror("خطأ - Error", "الرجاء إدخال كلمة المرور!\nPlease enter password!")
            return

        try:
            days = int(duration)
            if days <= 0:
                raise ValueError("Duration must be positive")
        except:
            messagebox.showerror("خطأ - Error", "المدة يجب أن تكون رقماً موجباً!\nDuration must be a positive number!")
            return

        try:
            max_users_int = int(max_users)
            if max_users_int <= 0:
                raise ValueError("Max users must be positive")
        except:
            messagebox.showerror("خطأ - Error", "عدد المستخدمين يجب أن يكون رقماً موجباً!\nMax users must be a positive number!")
            return

        # Generate license key with machine binding
        machine_id = self.get_machine_id()
        key = self.create_license_key(company, machine_id)
        expiry = (datetime.now() + timedelta(days=days)).strftime("%Y-%m-%d")
        created_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Save license with full details
        self.licenses[key] = {
            'company': company,
            'expiry': expiry,
            'created': created_date,
            'duration_days': days,
            'machine_id': machine_id,
            'license_type': 'Standard',
            'max_users': max_users_int,
            'features': ['all'],
            'status': 'active',
            'activation_count': 0,
            'last_check': None,
            'username': username,
            'password': password,
            'contact_email': email,
            'contact_phone': phone,
            'notes': notes
        }

        self.save_licenses()

        # ✅ NEW: Use the new sync methods that include license_hash and password_hash
        try:
            # Add to master database with all required fields
            success = self.add_license_to_master_db(
                license_key=key,
                client_company=company,
                admin_username=username,
                admin_password=password,
                client_email=email,
                client_phone=phone,
                expires_at=expiry,
                max_users=max_users_int,
                notes=notes
            )

            if success:
                # Create tenant database
                tenant_success = self.create_tenant_database(key, username, password)
                if tenant_success:
                    print(f"✅ License {key} created successfully with tenant database")
                else:
                    print(f"⚠️ License {key} created but tenant database creation failed")
            else:
                print(f"⚠️ License {key} saved to JSON but database sync failed")
                # Fallback to old method
                self.sync_license_to_database(key, self.licenses[key])
        except Exception as e:
            print(f"⚠️ Error in new sync method: {e}")
            # Fallback to old method
            self.sync_license_to_database(key, self.licenses[key])

        self.refresh_list()
        self.update_license_stats()  # Update statistics
        self.clear_form()

        # Show detailed license info
        self.show_license_details(key, self.licenses[key])

    def create_license_key(self, company, machine_id=""):
        # Create unique license key with multiple components
        timestamp = datetime.now().isoformat()
        random_part = secrets.token_hex(8)  # 8 bytes = 16 hex characters

        # Combine all parts
        data = f"{company}-{machine_id}-{timestamp}-{random_part}"
        hash_obj = hashlib.sha256(data.encode())

        # Format as XXXX-XXXX-XXXX-XXXX (16 characters total)
        full_key = hash_obj.hexdigest()[:16].upper()
        formatted_key = '-'.join([full_key[i:i+4] for i in range(0, 16, 4)])

        return formatted_key

    def get_machine_id(self):
        # Get unique machine identifier
        try:
            if platform.system() == "Windows":
                result = subprocess.run(['wmic', 'csproduct', 'get', 'uuid'],
                                      capture_output=True, text=True, timeout=5)
                machine_id = result.stdout.split('\n')[1].strip()
            else:
                machine_id = str(uuid.getnode())

            return hashlib.md5(machine_id.encode()).hexdigest()[:16].upper()
        except:
            return secrets.token_hex(8).upper()

    def show_license_details(self, key, data):
        # Create detailed license info window
        detail_window = tk.Toplevel(self.root)
        detail_window.title("تفاصيل الترخيص - License Details")
        detail_window.geometry("700x650")
        detail_window.configure(bg=self.colors['bg'])
        detail_window.resizable(False, False)

        # Center window
        detail_window.update_idletasks()
        x = (detail_window.winfo_screenwidth() // 2) - (700 // 2)
        y = (detail_window.winfo_screenheight() // 2) - (650 // 2)
        detail_window.geometry(f'700x650+{x}+{y}')

        # Header
        header = tk.Frame(detail_window, bg=self.colors['success'], height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="✅ تم إنشاء الترخيص بنجاح!",
            font=("Segoe UI", 20, "bold"),
            bg=self.colors['success'],
            fg='white'
        ).pack(expand=True)

        # Content with scrollbar
        content_frame = tk.Frame(detail_window, bg=self.colors['bg'])
        content_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

        # Canvas for scrolling
        canvas = tk.Canvas(content_frame, bg=self.colors['bg'], highlightthickness=0)
        scrollbar = ttk.Scrollbar(content_frame, orient="vertical", command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg=self.colors['bg'])

        scrollable_frame.bind(
            "<Configure>",
            lambda e: canvas.configure(scrollregion=canvas.bbox("all"))
        )

        canvas.create_window((0, 0), window=scrollable_frame, anchor="nw")
        canvas.configure(yscrollcommand=scrollbar.set)

        # License info card
        info_card = tk.Frame(scrollable_frame, bg=self.colors['card'], relief=tk.FLAT)
        info_card.pack(fill=tk.BOTH, expand=True)

        tk.Label(
            info_card,
            text="📋 معلومات الترخيص الكاملة - Complete License Information",
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(pady=(15, 10))

        # License details
        details = [
            ("🏢 اسم الشركة - Company Name", data.get('company', 'N/A')),
            ("🔑 مفتاح الترخيص - License Key", key),
            ("👤 اسم المستخدم - Username", data.get('username', 'N/A')),
            ("🔒 كلمة المرور - Password", data.get('password', 'N/A')),
            ("📅 تاريخ الإنشاء - Created Date", data.get('created', 'N/A')),
            ("⏱️ المدة - Duration", f"{data.get('duration_days', 0)} يوم / days"),
            ("📆 تاريخ الانتهاء - Expiry Date", data.get('expiry', 'N/A')),
            ("💻 معرف الجهاز - Machine ID", data.get('machine_id', 'N/A')),
            ("📊 نوع الترخيص - License Type", data.get('license_type', 'Standard')),
            ("👥 عدد المستخدمين - Max Users", str(data.get('max_users', 10))),
            ("✨ الميزات - Features", ', '.join(data.get('features', ['all']))),
            ("🔄 الحالة - Status", data.get('status', 'active').upper()),
            ("🔢 عدد التفعيلات - Activation Count", str(data.get('activation_count', 0))),
            ("📧 البريد الإلكتروني - Email", data.get('contact_email', 'N/A')),
            ("📞 رقم الهاتف - Phone", data.get('contact_phone', 'N/A')),
            ("📝 ملاحظات - Notes", data.get('notes', 'N/A'))
        ]

        for label, value in details:
            item_frame = tk.Frame(info_card, bg=self.colors['bg_light'])
            item_frame.pack(fill=tk.X, padx=20, pady=4)

            tk.Label(
                item_frame,
                text=label,
                font=("Segoe UI", 9),
                bg=self.colors['bg_light'],
                fg=self.colors['text_muted'],
                anchor='w'
            ).pack(anchor='w', padx=12, pady=(6, 2))

            # Make key copyable
            if "License Key" in label:
                key_label = tk.Label(
                    item_frame,
                    text=value,
                    font=("Courier New", 10, "bold"),
                    bg=self.colors['bg_light'],
                    fg=self.colors['accent'],
                    anchor='w',
                    cursor="hand2"
                )
                key_label.pack(anchor='w', padx=12, pady=(2, 6))
                key_label.bind("<Button-1>", lambda e, k=value: self.copy_to_clipboard(k, detail_window))
            else:
                tk.Label(
                    item_frame,
                    text=value,
                    font=("Segoe UI", 10, "bold"),
                    bg=self.colors['bg_light'],
                    fg=self.colors['text'],
                    anchor='w'
                ).pack(anchor='w', padx=12, pady=(2, 6))

        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # Buttons
        btn_frame = tk.Frame(detail_window, bg=self.colors['bg'])
        btn_frame.pack(pady=15)

        tk.Button(
            btn_frame,
            text="📋 نسخ المفتاح - Copy Key",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['accent'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.copy_to_clipboard(key, detail_window),
            padx=20,
            pady=10
        ).pack(side=tk.LEFT, padx=5)

        tk.Button(
            btn_frame,
            text="💾 حفظ كملف - Save to File",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: self.save_license_to_file(key, data),
            padx=20,
            pady=10
        ).pack(side=tk.LEFT, padx=5)

        tk.Button(
            btn_frame,
            text="❌ إغلاق - Close",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=detail_window.destroy,
            padx=20,
            pady=10
        ).pack(side=tk.LEFT, padx=5)

    def copy_to_clipboard(self, text, window=None):
        self.root.clipboard_clear()
        self.root.clipboard_append(text)
        messagebox.showinfo("نجح - Success", "تم نسخ المفتاح!\nKey copied to clipboard!", parent=window if window else self.root)

    def save_license_to_file(self, key, data):
        try:
            filename = f"License_{data['company'].replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            filepath = self.app_dir / filename

            with open(filepath, 'w', encoding='utf-8') as f:
                f.write("=" * 80 + "\n")
                f.write("DED MANAGEMENT SYSTEM - LICENSE CERTIFICATE\n")
                f.write("نظام إدارة DED - شهادة الترخيص\n")
                f.write("=" * 80 + "\n\n")

                f.write(f"Company Name / اسم الشركة: {data['company']}\n")
                f.write(f"\nLicense Key / مفتاح الترخيص:\n{key}\n\n")
                f.write(f"Created Date / تاريخ الإنشاء: {data['created']}\n")
                f.write(f"Duration / المدة: {data['duration_days']} days / يوم\n")
                f.write(f"Expiry Date / تاريخ الانتهاء: {data['expiry']}\n")
                f.write(f"Machine ID / معرف الجهاز: {data['machine_id']}\n")
                f.write(f"License Type / نوع الترخيص: {data['license_type']}\n")
                f.write(f"Max Users / عدد المستخدمين: {data['max_users']}\n")
                f.write(f"Features / الميزات: {', '.join(data['features'])}\n")
                f.write(f"Status / الحالة: {data['status']}\n")
                f.write(f"Activation Count / عدد التفعيلات: {data['activation_count']}\n\n")

                f.write("=" * 80 + "\n")
                f.write("IMPORTANT NOTES / ملاحظات مهمة:\n")
                f.write("=" * 80 + "\n")
                f.write("- Keep this license key safe / احتفظ بمفتاح الترخيص في مكان آمن\n")
                f.write("- Do not share with unauthorized users / لا تشاركه مع مستخدمين غير مصرح لهم\n")
                f.write("- Contact support for renewal / اتصل بالدعم للتجديد\n")
                f.write("- This license is bound to the machine ID / هذا الترخيص مرتبط بمعرف الجهاز\n")
                f.write("=" * 80 + "\n")

            messagebox.showinfo(
                "نجح - Success",
                f"تم حفظ الترخيص في:\nLicense saved to:\n\n{filepath}"
            )
        except Exception as e:
            messagebox.showerror("خطأ - Error", f"فشل حفظ الملف:\n{str(e)}")

    def clear_form(self):
        self.company_entry.delete(0, tk.END)
        self.duration_entry.delete(0, tk.END)
        self.duration_entry.insert(0, "365")
        self.username_entry.delete(0, tk.END)
        self.password_entry.delete(0, tk.END)
        self.email_entry.delete(0, tk.END)
        self.phone_entry.delete(0, tk.END)
        self.max_users_entry.delete(0, tk.END)
        self.max_users_entry.insert(0, "10")
        self.notes_entry.delete(0, tk.END)

    def refresh_list(self):
        print("=== refresh_list called ===")

        # Reload licenses
        self.licenses = self.load_licenses()
        print(f"Licenses in refresh_list: {len(self.licenses)}")

        # Calculate statistics
        total_count = len(self.licenses)
        active_count = 0
        suspended_count = 0
        expiring_soon_count = 0

        for key, data in self.licenses.items():
            status = data.get('status', 'active')
            expiry = data.get('expiry', 'N/A')

            try:
                expiry_date = datetime.strptime(expiry, "%Y-%m-%d")
                days_left = (expiry_date - datetime.now()).days

                if status == 'suspended':
                    suspended_count += 1
                elif days_left < 0:
                    suspended_count += 1  # Expired = suspended
                elif days_left <= 7:
                    expiring_soon_count += 1
                else:
                    active_count += 1
            except:
                if status == 'suspended':
                    suspended_count += 1
                else:
                    active_count += 1

        # Clear and recreate stat cards
        for widget in self.stat_cards_frame.winfo_children():
            widget.destroy()

        # Create 4 stat cards (like the image)
        stats = [
            {
                'title': 'تنتهي قريباً',
                'count': expiring_soon_count,
                'icon': '⚠️',
                'bg': '#ec4899',  # Pink
                'number_color': '#fce7f3'
            },
            {
                'title': 'التراخيص الموقوفة',
                'count': suspended_count,
                'icon': '⏸️',
                'bg': '#ef4444',  # Red
                'number_color': '#fee2e2'
            },
            {
                'title': 'التراخيص النشطة',
                'count': active_count,
                'icon': '✅',
                'bg': '#10b981',  # Green
                'number_color': '#d1fae5'
            },
            {
                'title': 'إجمالي التراخيص',
                'count': total_count,
                'icon': '⚙️',
                'bg': '#8b5cf6',  # Purple
                'number_color': '#ede9fe'
            }
        ]

        for stat in stats:
            self.create_stat_card(self.stat_cards_frame, stat)

        # Update count label
        self.license_count_label.config(text=f"جميع التراخيص {total_count}")

        # Clear existing license cards
        for widget in self.licenses_grid_frame.winfo_children():
            widget.destroy()

        if not self.licenses:
            print("No licenses - showing empty state")
            # Show beautiful empty state
            empty_container = tk.Frame(self.licenses_grid_frame, bg=self.colors['bg'])
            empty_container.pack(fill=tk.BOTH, expand=True, pady=100)

            tk.Label(
                empty_container,
                text="📋",
                font=("Segoe UI", 80),
                bg=self.colors['bg'],
                fg=self.colors['text_muted']
            ).pack(pady=20)

            tk.Label(
                empty_container,
                text="لا توجد تراخيص مسجلة",
                font=("Segoe UI", 24, "bold"),
                bg=self.colors['bg'],
                fg=self.colors['text']
            ).pack()

            tk.Label(
                empty_container,
                text="قم بإضافة ترخيص جديد من الأعلى",
                font=("Segoe UI", 14),
                bg=self.colors['bg'],
                fg=self.colors['text_gray']
            ).pack(pady=10)
            return

        print(f"Creating cards for {len(self.licenses)} licenses")
        # Create grid layout - 3 columns (like the image)
        row_frame = None
        for idx, (key, data) in enumerate(self.licenses.items()):
            print(f"Creating card {idx} for {key}")
            if idx % 3 == 0:
                # Create new row every 3 cards
                row_frame = tk.Frame(self.licenses_grid_frame, bg=self.colors['bg'])
                row_frame.pack(fill=tk.X, pady=10)

            self.create_license_card_new(row_frame, key, data)
        print("=== refresh_list completed ===")

    def create_stat_card(self, parent, stat):
        """Create a beautiful stat card (like the image) - COMPACT VERSION"""
        card = tk.Frame(parent, bg=stat['bg'], relief=tk.FLAT)
        card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)

        # Card content with SMALLER padding
        content = tk.Frame(card, bg=stat['bg'])
        content.pack(fill=tk.BOTH, expand=True, padx=15, pady=15)

        # SMALLER number at top
        tk.Label(
            content,
            text=str(stat['count']),
            font=("Segoe UI", 32, "bold"),  # Reduced from 48
            bg=stat['bg'],
            fg='white'
        ).pack(pady=(5, 10))

        # Icon and title at bottom
        bottom = tk.Frame(content, bg=stat['bg'])
        bottom.pack(fill=tk.X)

        tk.Label(
            bottom,
            text=stat['icon'],
            font=("Segoe UI", 14),  # Reduced from 20
            bg=stat['bg'],
            fg='white'
        ).pack(side=tk.RIGHT, padx=(5, 0))

        tk.Label(
            bottom,
            text=stat['title'],
            font=("Segoe UI", 10, "bold"),  # Reduced from 14
            bg=stat['bg'],
            fg='white'
        ).pack(side=tk.RIGHT)

    def create_license_card_new(self, parent_row, license_key, data):
        """Create a license card in 3-column grid (like the image)"""
        company = data.get('company', 'N/A')
        status = data.get('status', 'active')
        expiry = data.get('expiry', 'N/A')

        # Calculate days left
        try:
            expiry_date = datetime.strptime(expiry, "%Y-%m-%d")
            days_left = (expiry_date - datetime.now()).days
        except:
            days_left = 0

        # Determine status badge
        if status == 'suspended':
            status_text = "موقوف"
            status_bg = "#ef4444"  # Red
        elif days_left < 0:
            status_text = "منتهي"
            status_bg = "#ef4444"  # Red
        else:
            status_text = "نشط"
            status_bg = "#10b981"  # Green

        # Card container
        card = tk.Frame(parent_row, bg='#1e293b', relief=tk.FLAT)  # Dark blue/purple
        card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10)

        # Card content
        content = tk.Frame(card, bg='#1e293b')
        content.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)

        # Status badge in top-left
        status_badge = tk.Label(
            content,
            text=status_text,
            font=("Segoe UI", 11, "bold"),
            bg=status_bg,
            fg='white',
            padx=15,
            pady=5
        )
        status_badge.pack(anchor='w', pady=(0, 15))

        # License key section
        tk.Label(
            content,
            text="🔑 مفتاح الترخيص",
            font=("Segoe UI", 11),
            bg='#1e293b',
            fg='#94a3b8'
        ).pack(anchor='e', pady=(0, 8))

        # License key in rounded box
        key_box = tk.Frame(content, bg='#334155', relief=tk.FLAT)
        key_box.pack(fill=tk.X, pady=(0, 15))

        display_key = f"{license_key[:20]}..." if len(license_key) > 23 else license_key
        tk.Label(
            key_box,
            text=display_key,
            font=("Consolas", 9),
            bg='#334155',
            fg='white',
            padx=12,
            pady=8
        ).pack()

        # Action buttons container
        buttons_frame = tk.Frame(content, bg='#1e293b')
        buttons_frame.pack(fill=tk.X, pady=(10, 0))

        # Button style
        btn_style = {
            'font': ("Segoe UI", 9, "bold"),
            'relief': tk.FLAT,
            'cursor': "hand2",
            'padx': 8,
            'pady': 6,
            'borderwidth': 0
        }

        # Row 1: نسخ، التفاصيل، المستخدم
        row1 = tk.Frame(buttons_frame, bg='#1e293b')
        row1.pack(fill=tk.X, pady=(0, 5))

        # نسخ - Copy
        tk.Button(
            row1,
            text="📋 نسخ",
            bg='#3b82f6',
            fg='white',
            activebackground='#2563eb',
            activeforeground='white',
            command=lambda: self.copy_license_key(license_key),
            **btn_style
        ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

        # التفاصيل - Details
        tk.Button(
            row1,
            text="📄 التفاصيل",
            bg='#8b5cf6',
            fg='white',
            activebackground='#7c3aed',
            activeforeground='white',
            command=lambda: self.show_license_details(license_key, data),
            **btn_style
        ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

        # المستخدم - User
        tk.Button(
            row1,
            text="👤 المستخدم",
            bg='#06b6d4',
            fg='white',
            activebackground='#0891b2',
            activeforeground='white',
            command=lambda: self.show_user_info(license_key, data),
            **btn_style
        ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

        # Row 2: حذف، تمديد
        row2 = tk.Frame(buttons_frame, bg='#1e293b')
        row2.pack(fill=tk.X, pady=(0, 5))

        # حذف - Delete
        tk.Button(
            row2,
            text="🗑️ حذف",
            bg='#ef4444',
            fg='white',
            activebackground='#dc2626',
            activeforeground='white',
            command=lambda: self.delete_license(license_key),
            **btn_style
        ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

        # تمديد - Extend
        tk.Button(
            row2,
            text="⏰ تمديد",
            bg='#f59e0b',
            fg='white',
            activebackground='#d97706',
            activeforeground='white',
            command=lambda: self.extend_license(license_key, data),
            **btn_style
        ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

        # Row 3: توقيف / تفعيل (conditional)
        row3 = tk.Frame(buttons_frame, bg='#1e293b')
        row3.pack(fill=tk.X)

        if status == 'suspended':
            # تفعيل - Activate
            tk.Button(
                row3,
                text="✅ تفعيل",
                bg='#10b981',
                fg='white',
                activebackground='#059669',
                activeforeground='white',
                command=lambda: self.activate_license(license_key),
                **btn_style
            ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)
        else:
            # توقيف - Suspend
            tk.Button(
                row3,
                text="⏸️ توقيف",
                bg='#f97316',
                fg='white',
                activebackground='#ea580c',
                activeforeground='white',
                command=lambda: self.suspend_license(license_key),
                **btn_style
            ).pack(side=tk.RIGHT, padx=2, expand=True, fill=tk.X)

    def create_license_card(self, parent_row, license_key, data, column):
        """Create a modern beautiful card for a license in grid layout"""
        print(f"Creating card for {data.get('company', 'N/A')}")
        company = data.get('company', 'N/A')
        expiry = data.get('expiry', 'N/A')
        status = data.get('status', 'active')
        username = data.get('username', data.get('contact_email', 'N/A'))
        phone = data.get('phone', data.get('contact_phone', 'N/A'))
        email = data.get('email', data.get('contact_email', 'N/A'))
        print(f"Card data: company={company}, expiry={expiry}, status={status}")

        # Calculate days left and status
        try:
            expiry_date = datetime.strptime(expiry, "%Y-%m-%d")
            days_left = (expiry_date - datetime.now()).days

            if days_left < 0:
                status_text = "منتهي"
                status_subtext = f"منذ {abs(days_left)} يوم"
                status_icon = "❌"
                status_bg = self.colors['danger']
                card_accent = self.colors['danger']
            elif status == 'suspended':
                status_text = "معلق"
                status_subtext = "Suspended"
                status_icon = "⏸️"
                status_bg = self.colors['warning']
                card_accent = self.colors['warning']
            elif days_left <= 7:
                status_text = "ينتهي قريباً"
                status_subtext = f"{days_left} يوم متبقي"
                status_icon = "⚠️"
                status_bg = self.colors['danger']
                card_accent = self.colors['danger']
            elif days_left <= 30:
                status_text = "نشط"
                status_subtext = f"{days_left} يوم متبقي"
                status_icon = "⚠️"
                status_bg = self.colors['warning']
                card_accent = self.colors['warning']
            else:
                status_text = "نشط"
                status_subtext = f"{days_left} يوم متبقي"
                status_icon = "✅"
                status_bg = self.colors['success']
                card_accent = self.colors['success']
        except:
            status_text = "غير معروف"
            status_subtext = "Unknown"
            status_icon = "❓"
            status_bg = self.colors['text_gray']
            card_accent = self.colors['border']

        # Simple card container
        card = tk.Frame(
            parent_row,
            bg=self.colors['card'],
            relief=tk.RAISED,
            borderwidth=1
        )
        card.pack(fill=tk.X, padx=10, pady=5)

        # Make card clickable
        def select_card(event=None):
            self.selected_license_key = license_key
            card.config(relief=tk.SUNKEN)

        card.bind("<Button-1>", select_card)

        # Status bar
        status_bar = tk.Frame(card, bg=status_bg, height=5)
        status_bar.pack(fill=tk.X)

        # Card content
        content = tk.Frame(card, bg=self.colors['card'])
        content.pack(fill=tk.BOTH, expand=True, padx=15, pady=10)

        # Company name and status
        header_row = tk.Frame(content, bg=self.colors['card'])
        header_row.pack(fill=tk.X, pady=(0, 10))

        tk.Label(
            header_row,
            text=f"{status_icon} {company}",
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['card'],
            fg=self.colors['text'],
            anchor='w'
        ).pack(side=tk.LEFT)

        tk.Label(
            header_row,
            text=status_text,
            font=("Segoe UI", 10, "bold"),
            bg=status_bg,
            fg='white',
            padx=10,
            pady=3
        ).pack(side=tk.RIGHT)

        # License key
        key_row = tk.Frame(content, bg=self.colors['card'])
        key_row.pack(fill=tk.X, pady=5)

        tk.Label(
            key_row,
            text="🔑 المفتاح:",
            font=("Segoe UI", 9),
            bg=self.colors['card'],
            fg=self.colors['text_gray']
        ).pack(side=tk.LEFT)

        display_key = f"{license_key[:30]}..." if len(license_key) > 30 else license_key
        tk.Label(
            key_row,
            text=display_key,
            font=("Consolas", 9),
            bg=self.colors['card'],
            fg=self.colors['text']
        ).pack(side=tk.LEFT, padx=5)

        # Details
        details_row = tk.Frame(content, bg=self.colors['card'])
        details_row.pack(fill=tk.X, pady=5)

        tk.Label(
            details_row,
            text=f"📅 {expiry}  |  👤 {username}  |  📞 {phone}",
            font=("Segoe UI", 9),
            bg=self.colors['card'],
            fg=self.colors['text_gray']
        ).pack(anchor='w')

        # Action buttons
        actions = tk.Frame(content, bg=self.colors['card'])
        actions.pack(fill=tk.X, pady=(10, 0))

        tk.Button(
            actions,
            text="📋 نسخ",
            font=("Segoe UI", 9),
            bg=self.colors['accent'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: [self.select_license_by_key(license_key), self.copy_key()],
            padx=10,
            pady=5
        ).pack(side=tk.LEFT, padx=2)

        tk.Button(
            actions,
            text="✏️ تعديل",
            font=("Segoe UI", 9),
            bg=self.colors['info'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: [self.select_license_by_key(license_key), self.edit_license()],
            padx=10,
            pady=5
        ).pack(side=tk.LEFT, padx=2)

        tk.Button(
            actions,
            text="🔄 تمديد",
            font=("Segoe UI", 9),
            bg=self.colors['warning'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: [self.select_license_by_key(license_key), self.extend_license()],
            padx=10,
            pady=5
        ).pack(side=tk.LEFT, padx=2)

        if status == 'suspended':
            tk.Button(
                actions,
                text="✅ تفعيل",
                font=("Segoe UI", 9),
                bg=self.colors['success'],
                fg='white',
                relief=tk.FLAT,
                cursor="hand2",
                command=lambda: [self.select_license_by_key(license_key), self.activate_license()],
                padx=10,
                pady=5
            ).pack(side=tk.LEFT, padx=2)
        else:
            tk.Button(
                actions,
                text="⏸️ إيقاف",
                font=("Segoe UI", 9),
                bg=self.colors['warning_hover'],
                fg='white',
                relief=tk.FLAT,
                cursor="hand2",
                command=lambda: [self.select_license_by_key(license_key), self.suspend_license()],
                padx=10,
                pady=5
            ).pack(side=tk.LEFT, padx=2)

        tk.Button(
            actions,
            text="🗑️ حذف",
            font=("Segoe UI", 9),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=lambda: [self.select_license_by_key(license_key), self.delete_license()],
            padx=10,
            pady=5
        ).pack(side=tk.LEFT, padx=2)

    def select_license_by_key(self, license_key):
        """Helper to select a license by its key"""
        self.selected_license_key = license_key


    def update_license_stats(self):
        """Update license statistics in the app tab"""
        # Reload licenses
        self.licenses = self.load_licenses()

        # Count statistics
        total = len(self.licenses)
        active = 0
        suspended = 0
        expired = 0
        recent_list = []

        for key, data in self.licenses.items():
            company = data.get('company', 'N/A')
            expiry = data.get('expiry', 'N/A')
            status = data.get('status', 'active')

            # Check expiry
            try:
                expiry_date = datetime.strptime(expiry, "%Y-%m-%d")
                days_left = (expiry_date - datetime.now()).days

                if days_left < 0:
                    expired += 1
                    status_icon = "❌"
                    status_text = "منتهي"
                elif status == 'suspended':
                    suspended += 1
                    status_icon = "⏸️"
                    status_text = "معلق"
                else:
                    active += 1
                    if days_left <= 7:
                        status_icon = "🔴"
                    elif days_left <= 30:
                        status_icon = "⚠️"
                    else:
                        status_icon = "✅"
                    status_text = f"نشط ({days_left} يوم)"
            except:
                status_icon = "❓"
                status_text = "غير معروف"

            # Add to recent list (show last 5)
            recent_list.append(f"{status_icon} {company} - {status_text}")

        # Update stat labels
        self.total_licenses_label.config(text=str(total))
        self.active_licenses_label.config(text=str(active))
        self.suspended_licenses_label.config(text=str(suspended))
        self.expired_licenses_label.config(text=str(expired))

        # Update recent licenses listbox
        self.recent_licenses_listbox.delete(0, tk.END)

        if not recent_list:
            self.recent_licenses_listbox.insert(tk.END, "📭 لا توجد تراخيص - No licenses yet")
        else:
            # Show last 5 licenses
            for item in recent_list[-5:]:
                self.recent_licenses_listbox.insert(0, item)

        # Also refresh the license cards if we're on license tab
        if hasattr(self, 'licenses_canvas'):
            self.refresh_list()

    def copy_key(self):
        if not self.selected_license_key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        self.root.clipboard_clear()
        self.root.clipboard_append(self.selected_license_key)

        # Show key in message
        messagebox.showinfo(
            "نجح - Success",
            f"تم نسخ المفتاح الكامل!\nFull key copied to clipboard!\n\n{self.selected_license_key}"
        )

    def delete_license(self, license_key=None):
        """Delete a license with confirmation"""
        key = license_key or self.selected_license_key
        if not key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        license_data = self.licenses.get(key)
        if not license_data:
            messagebox.showerror("خطأ - Error", "لم يتم العثور على الترخيص!\nLicense not found!")
            return

        company = license_data.get('company', 'N/A')

        # Show detailed confirmation
        confirm_msg = (
            f"هل أنت متأكد من حذف هذا الترخيص؟\n"
            f"Are you sure you want to delete this license?\n\n"
            f"الشركة - Company: {company}\n"
            f"تاريخ الإنشاء - Created: {license_data.get('created', 'N/A')}\n"
            f"تاريخ الانتهاء - Expiry: {license_data.get('expiry', 'N/A')}\n\n"
            f"هذا الإجراء لا يمكن التراجع عنه!\n"
            f"This action cannot be undone!"
        )

        if messagebox.askyesno("تأكيد الحذف - Confirm Deletion", confirm_msg):
            del self.licenses[key]
            self.save_licenses()
            self.selected_license_key = None
            self.refresh_list()
            self.update_license_stats()
            messagebox.showinfo("نجح - Success", "تم حذف الترخيص بنجاح!\nLicense deleted successfully!")

    def copy_license_key(self, license_key):
        """Copy license key to clipboard"""
        self.root.clipboard_clear()
        self.root.clipboard_append(license_key)
        self.root.update()
        messagebox.showinfo("نجح - Success", "تم نسخ مفتاح الترخيص!\nLicense key copied to clipboard!")

    def show_license_details(self, license_key, data):
        """Show detailed license information in a popup"""
        dialog = tk.Toplevel(self.root)
        dialog.title("📄 تفاصيل الترخيص - License Details")
        dialog.geometry("600x700")
        dialog.configure(bg=self.colors['bg'])
        dialog.resizable(False, False)
        dialog.transient(self.root)
        dialog.grab_set()

        # Header
        header = tk.Frame(dialog, bg='#8b5cf6', height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="📄 تفاصيل الترخيص",
            font=("Segoe UI", 20, "bold"),
            bg='#8b5cf6',
            fg='white'
        ).pack(expand=True)

        # Content
        content = tk.Frame(dialog, bg=self.colors['bg'])
        content.pack(fill=tk.BOTH, expand=True, padx=30, pady=30)

        # Details
        details = [
            ("🏢 الشركة - Company", data.get('company', 'N/A')),
            ("👤 اسم المستخدم - Username", data.get('username', 'N/A')),
            ("📧 البريد - Email", data.get('email', 'N/A')),
            ("📱 الهاتف - Phone", data.get('phone', 'N/A')),
            ("🔑 مفتاح الترخيص - License Key", license_key),
            ("📅 تاريخ الإنشاء - Created", data.get('created', 'N/A')),
            ("⏰ تاريخ الانتهاء - Expiry", data.get('expiry', 'N/A')),
            ("👥 عدد المستخدمين - Max Users", str(data.get('max_users', 'N/A'))),
            ("📊 الحالة - Status", data.get('status', 'active')),
            ("📝 ملاحظات - Notes", data.get('notes', 'N/A'))
        ]

        for label, value in details:
            frame = tk.Frame(content, bg=self.colors['bg'])
            frame.pack(fill=tk.X, pady=8)

            tk.Label(
                frame,
                text=label,
                font=("Segoe UI", 10, "bold"),
                bg=self.colors['bg'],
                fg=self.colors['text'],
                anchor='e'
            ).pack(fill=tk.X, pady=(0, 3))

            value_frame = tk.Frame(frame, bg=self.colors['card'])
            value_frame.pack(fill=tk.X)

            tk.Label(
                value_frame,
                text=value,
                font=("Segoe UI", 10),
                bg=self.colors['card'],
                fg=self.colors['text'],
                anchor='w',
                wraplength=500
            ).pack(fill=tk.X, padx=15, pady=10)

        # Close button
        tk.Button(
            content,
            text="✅ إغلاق - Close",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['primary'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=dialog.destroy,
            padx=30,
            pady=12
        ).pack(pady=(20, 0), fill=tk.X)

    def show_user_info(self, license_key, data):
        """Show user information"""
        username = data.get('username', 'N/A')
        email = data.get('email', 'N/A')
        phone = data.get('phone', 'N/A')
        company = data.get('company', 'N/A')

        msg = (
            f"معلومات المستخدم - User Information\n\n"
            f"👤 اسم المستخدم - Username:\n{username}\n\n"
            f"🏢 الشركة - Company:\n{company}\n\n"
            f"📧 البريد - Email:\n{email}\n\n"
            f"📱 الهاتف - Phone:\n{phone}"
        )

        messagebox.showinfo("👤 معلومات المستخدم - User Info", msg)

    def extend_license(self, license_key, data):
        """Extend license duration"""
        dialog = tk.Toplevel(self.root)
        dialog.title("⏰ تمديد الترخيص - Extend License")
        dialog.geometry("500x350")
        dialog.configure(bg=self.colors['bg'])
        dialog.resizable(False, False)
        dialog.transient(self.root)
        dialog.grab_set()

        # Header
        header = tk.Frame(dialog, bg='#f59e0b', height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="⏰ تمديد الترخيص",
            font=("Segoe UI", 20, "bold"),
            bg='#f59e0b',
            fg='white'
        ).pack(expand=True)

        # Content
        content = tk.Frame(dialog, bg=self.colors['bg'])
        content.pack(fill=tk.BOTH, expand=True, padx=30, pady=30)

        # Current expiry
        current_expiry = data.get('expiry', 'N/A')
        tk.Label(
            content,
            text=f"تاريخ الانتهاء الحالي - Current Expiry:\n{current_expiry}",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(pady=(0, 20))

        # Days to extend
        tk.Label(
            content,
            text="عدد الأيام للتمديد - Days to Extend:",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(anchor='e', pady=(0, 5))

        days_entry = tk.Entry(
            content,
            font=("Segoe UI", 12),
            relief=tk.FLAT,
            bg=self.colors['card'],
            fg=self.colors['text'],
            insertbackground=self.colors['text']
        )
        days_entry.insert(0, "30")
        days_entry.pack(fill=tk.X, ipady=10, ipadx=10, pady=(0, 20))

        def do_extend():
            try:
                days = int(days_entry.get())
                if days <= 0:
                    messagebox.showerror("خطأ - Error", "الرجاء إدخال عدد أيام صحيح!\nPlease enter a valid number of days!")
                    return

                # Calculate new expiry
                current_date = datetime.strptime(current_expiry, "%Y-%m-%d")
                new_date = current_date + timedelta(days=days)
                new_expiry = new_date.strftime("%Y-%m-%d")

                # Update license
                self.licenses[license_key]['expiry'] = new_expiry
                self.save_licenses()
                self.refresh_list()
                self.update_license_stats()

                messagebox.showinfo(
                    "نجح - Success",
                    f"تم تمديد الترخيص بنجاح!\nLicense extended successfully!\n\n"
                    f"التاريخ الجديد - New Expiry:\n{new_expiry}"
                )
                dialog.destroy()
            except ValueError:
                messagebox.showerror("خطأ - Error", "الرجاء إدخال رقم صحيح!\nPlease enter a valid number!")

        # Buttons
        btn_frame = tk.Frame(content, bg=self.colors['bg'])
        btn_frame.pack(fill=tk.X, pady=(10, 0))

        tk.Button(
            btn_frame,
            text="✅ تمديد - Extend",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=do_extend,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))

        tk.Button(
            btn_frame,
            text="❌ إلغاء - Cancel",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['text_gray'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=dialog.destroy,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))

    def suspend_license(self, license_key):
        """Suspend a license"""
        if messagebox.askyesno(
            "تأكيد - Confirm",
            "هل تريد توقيف هذا الترخيص؟\nDo you want to suspend this license?"
        ):
            self.licenses[license_key]['status'] = 'suspended'
            self.save_licenses()
            self.refresh_list()
            self.update_license_stats()
            messagebox.showinfo("نجح - Success", "تم توقيف الترخيص!\nLicense suspended!")

    def activate_license(self, license_key):
        """Activate a suspended license"""
        if messagebox.askyesno(
            "تأكيد - Confirm",
            "هل تريد تفعيل هذا الترخيص؟\nDo you want to activate this license?"
        ):
            self.licenses[license_key]['status'] = 'active'
            self.save_licenses()
            self.refresh_list()
            self.update_license_stats()
            messagebox.showinfo("نجح - Success", "تم تفعيل الترخيص!\nLicense activated!")

    def activate_license(self):
        if not self.selected_license_key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        license_data = self.licenses.get(self.selected_license_key)
        if not license_data:
            messagebox.showerror("خطأ - Error", "لم يتم العثور على الترخيص!\nLicense not found!")
            return

        company = license_data.get('company', 'N/A')

        # Use License Control if available
        if self.license_control:
            try:
                success, message = self.license_control.activate_license(self.selected_license_key)
                if success:
                    # Update local JSON as well
                    self.licenses[self.selected_license_key]['status'] = 'active'
                    self.save_licenses()
                    self.refresh_list()
                    self.update_license_stats()
                    messagebox.showinfo("نجح - Success", f"✅ {message}\n\nتم تفعيل ترخيص {company}!")
                else:
                    messagebox.showerror("خطأ - Error", message)
            except Exception as e:
                messagebox.showerror("خطأ - Error", f"فشل التفعيل:\n{str(e)}")
        else:
            # Fallback to old method
            self.licenses[self.selected_license_key]['status'] = 'active'
            self.save_licenses()
            self.sync_license_to_database(self.selected_license_key, self.licenses[self.selected_license_key])
            self.refresh_list()
            self.update_license_stats()
            messagebox.showinfo("نجح - Success", f"تم تفعيل ترخيص {company}!\nLicense activated for {company}!")

    def suspend_license(self):
        if not self.selected_license_key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        license_data = self.licenses.get(self.selected_license_key)
        if not license_data:
            messagebox.showerror("خطأ - Error", "لم يتم العثور على الترخيص!\nLicense not found!")
            return

        company = license_data.get('company', 'N/A')

        # Ask for suspension reason
        reason = simpledialog.askstring(
            "سبب الإيقاف - Suspension Reason",
            "الرجاء إدخال سبب إيقاف الترخيص:\nPlease enter suspension reason:",
            initialvalue="تم الإيقاف من لوحة التحكم"
        )

        if reason is None:  # User cancelled
            return

        # Use License Control if available
        if self.license_control:
            try:
                success, message = self.license_control.suspend_license(self.selected_license_key, reason)
                if success:
                    # Update local JSON as well
                    self.licenses[self.selected_license_key]['status'] = 'suspended'
                    self.save_licenses()
                    self.refresh_list()
                    self.update_license_stats()
                    messagebox.showinfo("نجح - Success", f"⏸️ {message}\n\nتم إيقاف ترخيص {company}!")
                else:
                    messagebox.showerror("خطأ - Error", message)
            except Exception as e:
                messagebox.showerror("خطأ - Error", f"فشل الإيقاف:\n{str(e)}")
        else:
            # Fallback to old method
            self.licenses[self.selected_license_key]['status'] = 'suspended'
            self.save_licenses()
            self.sync_license_to_database(self.selected_license_key, self.licenses[self.selected_license_key])
            self.refresh_list()
            self.update_license_stats()
            messagebox.showinfo("نجح - Success", f"تم إيقاف ترخيص {company} مؤقتاً!\nLicense suspended for {company}!")

    def edit_license(self):
        if not self.selected_license_key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        license_data = self.licenses.get(self.selected_license_key)
        if not license_data:
            messagebox.showerror("خطأ - Error", "لم يتم العثور على الترخيص!\nLicense not found!")
            return

        # Create edit window
        self.show_edit_window(self.selected_license_key, license_data)

    def extend_license(self):
        """Extend License Duration"""
        if not self.selected_license_key:
            messagebox.showwarning("Warning", "Please select a license!")
            return

        license_data = self.licenses.get(self.selected_license_key)
        if not license_data:
            messagebox.showerror("Error", "License not found!")
            return

        company = license_data.get('company', 'N/A')
        license_key = self.selected_license_key

        extend_window = tk.Toplevel(self.root)
        extend_window.title("Extend License Duration")
        extend_window.geometry("600x400")
        extend_window.configure(bg=self.colors['bg'])
        extend_window.transient(self.root)
        extend_window.grab_set()

        extend_window.update_idletasks()
        x = (extend_window.winfo_screenwidth() // 2) - (600 // 2)
        y = (extend_window.winfo_screenheight() // 2) - (400 // 2)
        extend_window.geometry('600x400+{}+{}'.format(x, y))

        header = tk.Frame(extend_window, bg=self.colors['warning'], height=80)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="Extend License Duration",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['warning'],
            fg='white'
        ).pack(expand=True)

        content_frame = tk.Frame(extend_window, bg=self.colors['bg'])
        content_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=30)

        tk.Label(
            content_frame,
            text="Company: {}".format(company),
            font=("Segoe UI", 14, "bold"),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(pady=10)

        current_expiry = self.licenses[license_key].get('expiry', 'N/A')
        tk.Label(
            content_frame,
            text="Current Expiry: {}".format(current_expiry),
            font=("Segoe UI", 12),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(pady=10)

        tk.Label(
            content_frame,
            text="Days to Extend:",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).pack(pady=(20, 10))

        days_entry = tk.Entry(
            content_frame,
            font=("Segoe UI", 14),
            width=20,
            relief=tk.FLAT,
            bg=self.colors['card'],
            fg=self.colors['text'],
            insertbackground=self.colors['text']
        )
        days_entry.pack(pady=10, ipady=10)
        days_entry.insert(0, "30")
        days_entry.focus()

        btn_frame = tk.Frame(content_frame, bg=self.colors['bg'])
        btn_frame.pack(pady=30)

        def perform_extend():
            try:
                days = int(days_entry.get())
                if days <= 0:
                    messagebox.showerror("Error", "Please enter a valid number of days!")
                    return

                # Use License Control if available
                if self.license_control:
                    success, message = self.license_control.extend_license(license_key, days)
                    if success:
                        # Update local JSON as well
                        from datetime import datetime, timedelta
                        current_date = datetime.strptime(current_expiry, "%Y-%m-%d")
                        new_date = current_date + timedelta(days=days)
                        new_expiry = new_date.strftime("%Y-%m-%d")

                        self.licenses[license_key]['expiry'] = new_expiry
                        self.licenses[license_key]['duration_days'] = self.licenses[license_key].get('duration_days', 365) + days
                        self.save_licenses()
                        self.refresh_list()
                        self.update_license_stats()

                        extend_window.destroy()
                        messagebox.showinfo("نجح - Success", f"📅 {message}")
                    else:
                        messagebox.showerror("خطأ - Error", message)
                else:
                    # Fallback to old method
                    from datetime import datetime, timedelta
                    current_date = datetime.strptime(current_expiry, "%Y-%m-%d")
                    new_date = current_date + timedelta(days=days)
                    new_expiry = new_date.strftime("%Y-%m-%d")

                    self.licenses[license_key]['expiry'] = new_expiry
                    self.licenses[license_key]['duration_days'] = self.licenses[license_key].get('duration_days', 365) + days

                    self.save_licenses()
                    self.sync_license_to_database(license_key, self.licenses[license_key])
                    self.refresh_list()

                    extend_window.destroy()
                    newline = chr(10)
                    success_msg = "License extended successfully!" + newline + newline + "New Date: " + new_expiry
                    messagebox.showinfo("Success", success_msg)
            except ValueError:
                messagebox.showerror("Error", "Please enter a valid number!")
            except Exception as e:
                newline = chr(10)
                error_msg = "Failed to extend:" + newline + str(e)
                messagebox.showerror("Error", error_msg)

        tk.Button(
            btn_frame,
            text="Extend",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=perform_extend,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, padx=10)

        tk.Button(
            btn_frame,
            text="Cancel",
            font=("Segoe UI", 12, "bold"),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=extend_window.destroy,
            padx=30,
            pady=12
        ).pack(side=tk.LEFT, padx=10)


    def view_license_details(self):
        if not self.selected_license_key:
            messagebox.showwarning("تحذير - Warning", "الرجاء اختيار ترخيص!\nPlease select a license!")
            return

        license_data = self.licenses.get(self.selected_license_key)
        if not license_data:
            messagebox.showerror("خطأ - Error", "لم يتم العثور على الترخيص!\nLicense not found!")
            return

        full_key = self.selected_license_key
        company = license_data.get('company', 'N/A')

        if full_key:
            self.show_license_details(full_key, license_data)

    def show_edit_window(self, key, data):
        # Create edit window
        edit_window = tk.Toplevel(self.root)
        edit_window.title("تعديل الترخيص - Edit License")
        edit_window.geometry("600x550")
        edit_window.configure(bg=self.colors['bg'])
        edit_window.resizable(False, False)

        # Center window
        edit_window.update_idletasks()
        x = (edit_window.winfo_screenwidth() // 2) - (600 // 2)
        y = (edit_window.winfo_screenheight() // 2) - (550 // 2)
        edit_window.geometry(f'600x550+{x}+{y}')

        # Header
        header = tk.Frame(edit_window, bg=self.colors['info'], height=60)
        header.pack(fill=tk.X)
        header.pack_propagate(False)

        tk.Label(
            header,
            text="✏️ تعديل معلومات الترخيص - Edit License Information",
            font=("Segoe UI", 16, "bold"),
            bg=self.colors['info'],
            fg='white'
        ).pack(expand=True)

        # Form
        form_frame = tk.Frame(edit_window, bg=self.colors['bg'])
        form_frame.pack(fill=tk.BOTH, expand=True, padx=30, pady=20)

        # Company (read-only)
        tk.Label(
            form_frame,
            text="اسم الشركة - Company:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=0, column=0, sticky='e', padx=10, pady=8)

        company_label = tk.Label(
            form_frame,
            text=data.get('company', ''),
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['bg'],
            fg=self.colors['accent']
        )
        company_label.grid(row=0, column=1, sticky='w', padx=10, pady=8)

        # Username
        tk.Label(
            form_frame,
            text="اسم المستخدم - Username:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=1, column=0, sticky='e', padx=10, pady=8)

        username_var = tk.StringVar(value=data.get('username', ''))
        username_edit = tk.Entry(form_frame, textvariable=username_var, font=("Segoe UI", 11), width=30)
        username_edit.grid(row=1, column=1, sticky='w', padx=10, pady=8)

        # Password
        tk.Label(
            form_frame,
            text="كلمة المرور - Password:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=2, column=0, sticky='e', padx=10, pady=8)

        password_var = tk.StringVar(value=data.get('password', ''))
        password_edit = tk.Entry(form_frame, textvariable=password_var, font=("Segoe UI", 11), width=30, show="*")
        password_edit.grid(row=2, column=1, sticky='w', padx=10, pady=8)

        # Email
        tk.Label(
            form_frame,
            text="البريد الإلكتروني - Email:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=3, column=0, sticky='e', padx=10, pady=8)

        email_var = tk.StringVar(value=data.get('contact_email', ''))
        email_edit = tk.Entry(form_frame, textvariable=email_var, font=("Segoe UI", 11), width=30)
        email_edit.grid(row=3, column=1, sticky='w', padx=10, pady=8)

        # Phone
        tk.Label(
            form_frame,
            text="رقم الهاتف - Phone:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=4, column=0, sticky='e', padx=10, pady=8)

        phone_var = tk.StringVar(value=data.get('contact_phone', ''))
        phone_edit = tk.Entry(form_frame, textvariable=phone_var, font=("Segoe UI", 11), width=30)
        phone_edit.grid(row=4, column=1, sticky='w', padx=10, pady=8)

        # Max Users
        tk.Label(
            form_frame,
            text="عدد المستخدمين - Max Users:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=5, column=0, sticky='e', padx=10, pady=8)

        max_users_var = tk.StringVar(value=str(data.get('max_users', 10)))
        max_users_edit = tk.Entry(form_frame, textvariable=max_users_var, font=("Segoe UI", 11), width=30)
        max_users_edit.grid(row=5, column=1, sticky='w', padx=10, pady=8)

        # Notes
        tk.Label(
            form_frame,
            text="ملاحظات - Notes:",
            font=("Segoe UI", 11),
            bg=self.colors['bg'],
            fg=self.colors['text']
        ).grid(row=6, column=0, sticky='e', padx=10, pady=8)

        notes_var = tk.StringVar(value=data.get('notes', ''))
        notes_edit = tk.Entry(form_frame, textvariable=notes_var, font=("Segoe UI", 11), width=30)
        notes_edit.grid(row=6, column=1, sticky='w', padx=10, pady=8)

        # Save function
        def save_changes():
            try:
                max_users_int = int(max_users_var.get())
                if max_users_int <= 0:
                    raise ValueError()
            except:
                messagebox.showerror("خطأ - Error", "عدد المستخدمين يجب أن يكون رقماً موجباً!", parent=edit_window)
                return

            # Update license
            self.licenses[key]['username'] = username_var.get().strip()
            self.licenses[key]['password'] = password_var.get().strip()
            self.licenses[key]['contact_email'] = email_var.get().strip()
            self.licenses[key]['contact_phone'] = phone_var.get().strip()
            self.licenses[key]['max_users'] = max_users_int
            self.licenses[key]['notes'] = notes_var.get().strip()

            self.save_licenses()
            self.refresh_list()

            messagebox.showinfo("نجح - Success", "تم تحديث الترخيص بنجاح!\nLicense updated successfully!", parent=edit_window)
            edit_window.destroy()

        # Buttons
        btn_frame = tk.Frame(edit_window, bg=self.colors['bg'])
        btn_frame.pack(pady=20)

        tk.Button(
            btn_frame,
            text="💾 حفظ التغييرات - Save Changes",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['success'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=save_changes,
            padx=20,
            pady=10
        ).pack(side=tk.LEFT, padx=5)

        tk.Button(
            btn_frame,
            text="❌ إلغاء - Cancel",
            font=("Segoe UI", 11, "bold"),
            bg=self.colors['danger'],
            fg='white',
            relief=tk.FLAT,
            cursor="hand2",
            command=edit_window.destroy,
            padx=20,
            pady=10
        ).pack(side=tk.LEFT, padx=5)

    def sync_license_to_database(self, license_key, license_data):
        """Sync license to database by creating/updating user account"""
        try:
            # Use the MASTER database (licenses_master.db) - this is what the app uses!
            db_path = self.app_dir / "licenses_master.db"

            if not db_path.exists():
                print(f"⚠️ Master database not found at {db_path}")
                print(f"Creating new master database...")
                # Database will be created automatically by sqlite3.connect

            # Use timeout to avoid database locked errors
            conn = sqlite3.connect(str(db_path), timeout=30.0)
            cursor = conn.cursor()

            # Calculate expiry datetime
            expiry_date = datetime.strptime(license_data.get('expiry'), "%Y-%m-%d")

            # Insert or update license (using the correct column names from the database)
            cursor.execute("""
                INSERT OR REPLACE INTO licenses
                (license_key, client_name, client_company, client_email, client_phone,
                 machine_id, expires_at, license_type, max_users, max_branches,
                 is_active, is_suspended, notes, created_at, activated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                license_key,
                license_data.get('username'),  # client_name
                license_data.get('company'),   # client_company
                license_data.get('contact_email'),  # client_email
                license_data.get('contact_phone'),  # client_phone
                license_data.get('machine_id'),
                expiry_date,  # expires_at
                license_data.get('license_type', 'Standard'),
                license_data.get('max_users', 10),
                5,  # max_branches (default)
                1 if license_data.get('status') == 'active' else 0,  # is_active
                0,  # is_suspended
                license_data.get('notes'),
                datetime.now(),  # created_at
                datetime.now() if license_data.get('status') == 'active' else None  # activated_at
            ))

            # Get license ID
            license_id = cursor.lastrowid
            if license_id == 0:
                # License already exists, get its ID
                cursor.execute("SELECT id FROM licenses WHERE license_key = ?", (license_key,))
                result = cursor.fetchone()
                if result:
                    license_id = result[0]

            # Check if user exists
            username = license_data.get('username')
            cursor.execute("SELECT id FROM users WHERE username = ?", (username,))
            user_exists = cursor.fetchone()

            if not user_exists:
                # Create new user
                password_hash = generate_password_hash(license_data.get('password'))

                cursor.execute("""
                    INSERT INTO users
                    (username, email, password_hash, full_name, phone, is_active,
                     is_admin, language, license_id, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    username,
                    license_data.get('contact_email', f"{username}@example.com"),
                    password_hash,
                    license_data.get('company'),
                    license_data.get('contact_phone'),
                    1 if license_data.get('status') == 'active' else 0,
                    0,  # is_admin
                    'ar',  # language
                    license_id,
                    datetime.now()
                ))
                print(f"✅ Created user: {username} with license: {license_key[:20]}...")
            else:
                # Update existing user
                cursor.execute("""
                    UPDATE users
                    SET license_id = ?,
                        is_active = ?,
                        email = ?,
                        phone = ?
                    WHERE username = ?
                """, (
                    license_id,
                    1 if license_data.get('status') == 'active' else 0,
                    license_data.get('contact_email', f"{username}@example.com"),
                    license_data.get('contact_phone'),
                    username
                ))
                print(f"✅ Updated user: {username} with license: {license_key[:20]}...")

            conn.commit()
            conn.close()

            print(f"✅ License synced successfully to database!")
            print(f"   License Key: {license_key}")
            print(f"   Client: {license_data.get('company')}")
            print(f"   Username: {license_data.get('username')}")

        except Exception as e:
            print(f"❌ Error syncing license to database: {e}")
            import traceback
            traceback.print_exc()

    def apply_migration(self):
        """Apply database migration for license system"""
        try:
            # Check if database exists first
            db_path = self.app_dir / "erp_system.db"

            if not db_path.exists():
                messagebox.showerror(
                    "خطأ - Error",
                    "❌ قاعدة البيانات غير موجودة!\n"
                    "Database not found!\n\n"
                    "⚠️ يجب تشغيل التطبيق أولاً لإنشاء قاعدة البيانات:\n"
                    "You must start the application first to create the database:\n\n"
                    "1️⃣ اذهب إلى تبويب 'تشغيل التطبيق'\n"
                    "   Go to 'App Control' tab\n\n"
                    "2️⃣ اضغط على 'تشغيل التطبيق'\n"
                    "   Click 'Start Application'\n\n"
                    "3️⃣ انتظر حتى يتم تشغيل التطبيق\n"
                    "   Wait for the application to start\n\n"
                    "4️⃣ ثم ارجع وحاول مرة أخرى\n"
                    "   Then come back and try again"
                )
                return

            # Check if application is running
            if not self.is_running:
                response = messagebox.askyesno(
                    "تحذير - Warning",
                    "⚠️ التطبيق غير قيد التشغيل!\n"
                    "Application is not running!\n\n"
                    "يُفضل تشغيل التطبيق أولاً لضمان نجاح Migration.\n"
                    "It's recommended to start the application first.\n\n"
                    "هل تريد المتابعة على أي حال؟\n"
                    "Do you want to continue anyway?"
                )
                if not response:
                    return

            result = subprocess.run(
                [sys.executable, "apply_license_migration.py"],
                cwd=self.app_dir,
                capture_output=True,
                text=True,
                timeout=30
            )

            if result.returncode == 0 or "Migration completed successfully" in result.stdout:
                # Extract key information from output
                success_msg = "✅ تم تطبيق Migration بنجاح!\n✅ Migration applied successfully!\n\n"

                if "Licenses table already exists" in result.stdout:
                    success_msg += "✓ جدول التراخيص موجود بالفعل\n  Licenses table already exists\n\n"
                elif "Licenses table created" in result.stdout:
                    success_msg += "✓ تم إنشاء جدول التراخيص\n  Licenses table created\n\n"

                if "license_id column added" in result.stdout:
                    success_msg += "✓ تم إضافة عمود license_id لجدول المستخدمين\n  license_id column added to users table\n\n"
                elif "already has license_id column" in result.stdout:
                    success_msg += "✓ جدول المستخدمين يحتوي بالفعل على عمود license_id\n  Users table already has license_id column\n\n"

                success_msg += "🎉 يمكنك الآن استخدام نظام التراخيص!\n🎉 You can now use the license system!"

                messagebox.showinfo("نجح - Success", success_msg)
            else:
                messagebox.showerror(
                    "خطأ - Error",
                    f"❌ فشل تطبيق Migration!\nMigration failed!\n\n{result.stdout}\n{result.stderr}"

                )

        except Exception as e:
            messagebox.showerror(
                "خطأ - Error",
                f"❌ حدث خطأ!\nError occurred!\n\n{str(e)}"
            )

    def add_license_to_master_db(self, license_key, license_data):
        """Add license to licenses_master.db with all required fields"""
        try:
            db_path = self.app_dir / "licenses_master.db"
            conn = sqlite3.connect(str(db_path), timeout=30.0)
            cursor = conn.cursor()

            # Generate license hash
            license_hash = hashlib.sha256(license_key.encode()).hexdigest()

            # Generate password hash
            admin_password_hash = generate_password_hash(license_data.get('password', 'admin123'))

            # Calculate expiry datetime
            expiry_date = datetime.strptime(license_data.get('expiry'), "%Y-%m-%d")
            created_at = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

            # Insert license
            cursor.execute('''
                INSERT INTO licenses (
                    license_key, license_hash, client_name, client_company,
                    client_email, client_phone, machine_id, expires_at,
                    license_type, max_users, max_branches, is_active,
                    is_suspended, notes, created_at, admin_username, admin_password_hash
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                license_key,
                license_hash,
                license_data.get('username'),
                license_data.get('company'),
                license_data.get('contact_email', ''),
                license_data.get('contact_phone', ''),
                license_data.get('machine_id', ''),
                expiry_date,
                license_data.get('license_type', 'Standard'),
                license_data.get('max_users', 10),
                5,  # max_branches
                1 if license_data.get('status') == 'active' else 0,
                0,  # is_suspended
                license_data.get('notes', ''),
                created_at,
                license_data.get('username'),
                admin_password_hash
            ))

            conn.commit()
            conn.close()

            return True, "تم إضافة الترخيص بنجاح إلى قاعدة البيانات الرئيسية"

        except sqlite3.IntegrityError as e:
            if "UNIQUE constraint failed" in str(e):
                return False, "الترخيص موجود بالفعل في قاعدة البيانات"
            return False, f"خطأ في قاعدة البيانات: {str(e)}"
        except Exception as e:
            return False, f"خطأ: {str(e)}"

    def create_tenant_database(self, license_key, username, password):
        """Create tenant database for the license"""
        try:
            # Generate tenant database name
            tenant_key = license_key.replace('-', '').lower()
            tenant_db_path = self.app_dir / f"tenant_{tenant_key}.db"

            # Create tenant database
            conn = sqlite3.connect(str(tenant_db_path))
            cursor = conn.cursor()

            # Create users table
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS users (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    username TEXT UNIQUE NOT NULL,
                    password_hash TEXT NOT NULL,
                    role_id INTEGER,
                    is_active BOOLEAN DEFAULT 1,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
                )
            ''')

            # Create roles table
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS roles (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    name TEXT UNIQUE NOT NULL,
                    permissions TEXT
                )
            ''')

            # Add admin role
            cursor.execute('''
                INSERT OR IGNORE INTO roles (id, name, permissions)
                VALUES (1, 'admin', 'all')
            ''')

            # Add admin user
            password_hash = generate_password_hash(password)
            cursor.execute('''
                INSERT OR IGNORE INTO users (username, password_hash, role_id, is_active)
                VALUES (?, ?, 1, 1)
            ''', (username, password_hash))

            conn.commit()
            conn.close()

            print(f"✅ Tenant database created: tenant_{tenant_key}.db")
            print(f"   Username: {username}")
            print(f"   Password: {password}")

            return True

        except Exception as e:
            print(f"❌ Error creating tenant database: {e}")
            return False

    def sync_from_database(self):
        """Sync licenses from licenses_master.db to licenses.json"""
        try:
            db_path = self.app_dir / "licenses_master.db"
            if not db_path.exists():
                print("⚠️ licenses_master.db not found, skipping sync")
                return False

            conn = sqlite3.connect(str(db_path), timeout=30.0)
            cursor = conn.cursor()

            # Get all licenses from database
            cursor.execute("""
                SELECT license_key, client_company, client_name, client_email,
                       client_phone, expires_at, license_type, max_users,
                       is_active, is_suspended, notes, created_at, machine_id,
                       admin_username
                FROM licenses
            """)

            db_licenses = cursor.fetchall()
            conn.close()

            sync_count = 0
            new_count = 0
            updated_count = 0

            for row in db_licenses:
                (license_key, company, username, email, phone, expires_at,
                 license_type, max_users, is_active, is_suspended, notes,
                 created_at, machine_id, admin_username) = row

                # Convert expires_at to date string
                if expires_at:
                    try:
                        if isinstance(expires_at, str):
                            expiry_date = datetime.strptime(expires_at.split()[0], "%Y-%m-%d")
                        else:
                            expiry_date = expires_at
                        expiry = expiry_date.strftime("%Y-%m-%d")
                    except:
                        expiry = str(expires_at).split()[0] if expires_at else "N/A"
                else:
                    expiry = "N/A"

                # Determine status
                if is_suspended:
                    status = 'suspended'
                elif is_active:
                    status = 'active'
                else:
                    status = 'inactive'

                # Check if license exists in JSON
                if license_key in self.licenses:
                    # Update existing license
                    old_status = self.licenses[license_key].get('status')
                    old_expiry = self.licenses[license_key].get('expiry')

                    if old_status != status or old_expiry != expiry:
                        self.licenses[license_key].update({
                            'company': company or self.licenses[license_key].get('company', 'N/A'),
                            'expiry': expiry,
                            'status': status,
                            'license_type': license_type or 'Standard',
                            'max_users': max_users or 10,
                            'notes': notes or '',
                            'username': admin_username or username or self.licenses[license_key].get('username', ''),
                            'contact_email': email or '',
                            'contact_phone': phone or '',
                            'machine_id': machine_id or ''
                        })
                        updated_count += 1
                        print(f"🔄 Updated: {license_key} - {company} (Status: {status})")
                else:
                    # Add new license from database
                    self.licenses[license_key] = {
                        'company': company or 'N/A',
                        'expiry': expiry,
                        'created': created_at or datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        'duration_days': 365,
                        'machine_id': machine_id or '',
                        'license_type': license_type or 'Standard',
                        'max_users': max_users or 10,
                        'features': ['all'],
                        'status': status,
                        'activation_count': 0,
                        'last_check': None,
                        'username': admin_username or username or 'admin',
                        'password': 'admin123',  # Default password
                        'contact_email': email or '',
                        'contact_phone': phone or '',
                        'notes': notes or ''
                    }
                    new_count += 1
                    print(f"➕ Added: {license_key} - {company}")

                sync_count += 1

            # Save updated licenses
            if new_count > 0 or updated_count > 0:
                self.save_licenses()
                print(f"✅ Sync complete: {sync_count} total, {new_count} new, {updated_count} updated")
                return True
            else:
                print(f"✅ Sync complete: All {sync_count} licenses up to date")
                return False

        except Exception as e:
            print(f"❌ Error syncing from database: {e}")
            import traceback
            traceback.print_exc()
            return False

    def auto_sync(self):
        """Auto-sync licenses from database periodically"""
        try:
            if self.license_control:
                # Perform sync
                changed = self.sync_from_database()

                if changed:
                    # Refresh UI if changes were made
                    self.refresh_list()
                    self.update_license_stats()
                    print("🔄 Auto-sync: UI refreshed due to changes")
                else:
                    print("🔄 Auto-sync: No changes detected")

        except Exception as e:
            print(f"⚠️ Auto-sync error: {e}")

        # Schedule next sync
        self.root.after(self.auto_sync_interval, self.auto_sync)

    def manual_sync(self):
        """Manual sync triggered by user"""
        try:
            if not self.license_control:
                messagebox.showwarning(
                    "تحذير - Warning",
                    "نظام التحكم الخارجي غير متاح!\nExternal control system not available!"
                )
                return

            # Show progress
            print("🔄 Starting manual sync...")

            # Perform sync
            changed = self.sync_from_database()

            # Refresh UI
            self.refresh_list()
            self.update_license_stats()

            if changed:
                messagebox.showinfo(
                    "نجح - Success",
                    "✅ تمت المزامنة بنجاح!\nتم تحديث التراخيص من قاعدة البيانات.\n\n"
                    "Sync completed successfully!\nLicenses updated from database."
                )
            else:
                messagebox.showinfo(
                    "معلومات - Info",
                    "✅ المزامنة تمت بنجاح!\nجميع التراخيص محدثة.\n\n"
                    "Sync completed!\nAll licenses are up to date."
                )

        except Exception as e:
            messagebox.showerror(
                "خطأ - Error",
                f"❌ فشلت المزامنة!\nSync failed!\n\n{str(e)}"
            )

    def on_closing(self):
        if self.is_running:
            if messagebox.askyesno("تأكيد - Confirm", "التطبيق يعمل. هل تريد إيقافه والخروج?\nApp is running. Stop and exit?"):
                self.stop_app()
                self.root.destroy()
        else:
            self.root.destroy()

if __name__ == "__main__":
    root = tk.Tk()
    app = DEDControlPanel(root)
    root.mainloop()

